...
class MBPO:
    ...
    def __init__(self, train_kwargs, model_kwargs, TD3_kwargs):
        ...
        self.enable_MBPO = False#train_kwargs["enable_MBPO"]
        ...
        self.max_timesteps = 40#train_kwargs["max_timesteps"] #maximum real-env timestemps
        self.start_timesteps = 10#train_kwargs["start_timesteps"] #burn-in period
        self.batch_size = 3#train_kwargs["batch_size"]
    ...
    def model_rollout(self):
        ...
        for j in range(batch_pass):
            ...
            next_state, reward, done = self.fake_env.step()
    ...
    def prepare_mixed_batch(self):
        ...
        state, action, next_state, reward, done = self.replay_buffer_Model.sample(self.batch_size)
        # state = x
    ...
    def train(self):
        ...
        for t in range(int(self.max_timesteps)):
            ...
            self.replay_buffer_Model.add(state, action, next_state, reward, done)#state, action, next_state, reward, done

                # Perform model rollout and model training at appropriate timesteps 
            if t >= self.start_timesteps:
                #self.agent.train(self.memory, self.config['batch_size'])
                state, action, next_state, reward, done = self.prepare_mixed_batch()
                self.policy.train_on_batch(state, action, next_state, reward, done)
