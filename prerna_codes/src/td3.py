...
class TD3():
    ...
    def train_on_batch(self, state, action, next_state, reward, not_done):
        ...
        # Compute critic loss
        critic_loss = tf.losses.mean_squared_error(current_Q1, target_Q) + tf.losses.mean_squared_error(current_Q2, target_Q)

        # Optimize the critic
        # trainable_critic_variables = list(object_identity.ObjectIdentitySet(self._critic_network_1.trainable_variables +
        # self._critic_network_2.trainable_variables))
        with tf.GradientTape() as tape:
            tape.watch(critic_loss)
            critic_loss = tf.losses.mean_squared_error(current_Q1, target_Q) + tf.losses.mean_squared_error(current_Q2, target_Q)
